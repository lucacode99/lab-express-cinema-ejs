...
mongoose
  ...
  .then((x) => {
    ...
    return Movie.insertMany(movies) // in this model, insert the array of information that goes into the database
  })

  .catch((err) => {                 // falls es einen Fehler beim chaining gibt, zeigt der error mir das an!
    console.error("ERROR with seeding");
  })

  .finally(() => {
    console.log("Database closed successfully - nothing to do here")
    mongoose.connection.close()     // verbindung wieder schließen
  })
